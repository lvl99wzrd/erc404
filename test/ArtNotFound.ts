...
describe("ERC404", function () {
  ...
  async function deployArtNotFoundWithTokensInSecondSigner() {
    const f = await loadFixture(deployArtNotFound)
  ...
  async function deployArtNotFoundWithSomeTokensTransferredToRandomAddress() {
    const f = await loadFixture(deployArtNotFound)
  ...
  describe("#erc20TotalSupply", function () {
    it("Returns the correct total supply", async function () {
      const f = await loadFixture(
        deployArtNotFoundWithSomeTokensTransferredToRandomAddress,
      )

      expect(await f.contract.erc20TotalSupply()).to.eq(
        404n * f.deployConfig.units,
      )
    })
  })
    ...
    context(
      ...
      async function () {
        it("Handles the case of the receiver gaining a whole new token", async function () {
          const f = await loadFixture(deployArtNotFoundWithTokensInSecondSigner)
